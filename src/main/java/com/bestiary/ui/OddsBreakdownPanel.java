...
final class OddsBreakdownPanel {
    ...
    static JPanel build(OddsCalculator.Result r) {
        ...
        // Power Level: 7-stat average + HP at 1/6 weight + combat level at 1/10 weight (Prayer is the 7th stat)
        ...
        root.add(value(kvRow("Combat level (factual)  ÷ 10", body, new Color(120, 200, 120), "+" + Math.round(r.combatLevel / 10f)), nearWhite));
        root.add(value(kvRow("= Power Level  (" + sevenStats + " ÷ 7) + (" + r.hp + " ÷ 6) + (" + r.combatLevel + " ÷ 10)", body, Color.WHITE,
